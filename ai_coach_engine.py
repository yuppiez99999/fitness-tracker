...
def generate_strength_cycle(
    ...
    for phase, n_weeks in dist.items():
        ...
        for _ in range(n_weeks):
            ...
            top_set = WorkoutSet(
                weight=w, reps=cfg["reps"], sets=1, rpe_start=cfg["rpe"][0], rpe_end=cfg["rpe"][1], set_type="顶组"
            )
...
def review_training(log: TrainingLog, prev_log: Optional[TrainingLog] = None) -> ReviewResult:
    ...
    return ReviewResult(judgment=judgment, key_findings=findings, next_prescription=next_rx, progression_type=prog)
...
def generate_return_plan(days_off: int, last_weight: float = 0.0, movement: str = "主项") -> ReturnPlan:
    """生成停训接回三档方案
